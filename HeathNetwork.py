...
while i < len(sys.argv):
    ...
    if arg == "-ext" or arg == "--extinction_parameters":
        i+=1
        extinction_rate_mean = float(sys.argv[i])
        i+=1
        extinction_rate_shape = float(sys.argv[i])    
        if extinction_rate_mean==0:
            ext_used = False
    if arg == "-hgt" or arg == "--hgt_parameters":
        i+=1
        hgt_rate_mean = float(sys.argv[i])
        i+=1
        hgt_rate_shape = float(sys.argv[i])        
        hgt_used = True
    if arg == "-hyb" or arg == "--hybridization_factor":
        i+=1
        hybridization_left_bound  = float(sys.argv[i])
        i+=1
        hybridization_right_bound = float(sys.argv[i])
        i+=1
        hybridization_left_rate   = float(sys.argv[i])
        i+=1
        hybridization_right_rate  = float(sys.argv[i])
        hyb_used = True
    if arg == "-upd" or arg == "--update_shape_parameter":
        i+=1
        update_shape = float(sys.argv[i])
...
def CalculateNewRate(parent_rate,prior_mean,prior_shape):
...
def CalculateAllNewRates(parent_rates):
    sp_rate = CalculateNewRate(parent_rates[0],speciation_rate_mean,speciation_rate_shape)
    ext_rate = 0
    if ext_used:
        ext_rate = CalculateNewRate(parent_rates[1],extinction_rate_mean,extinction_rate_shape)
    hgt_rate = 0
    if hgt_used:
        hgt_rate = CalculateNewRate(parent_rates[2],hgt_rate_mean,hgt_rate_shape)
...
def CalculateNetwork():
    ...
    while len(leaves)>0 and ((not taxa_limit and current_time<time_limit) or len(leaves)+c_ext*no_of_extinct<taxa_limit) :
        ...
        if random_number < current_speciation_rate / total_rate:
            ...
            #rates
            leaf_rates[current_node]=CalculateAllNewRates(leaf_rates[splitting_leaf])
            leaf_rates[current_node+1]=CalculateAllNewRates(leaf_rates[splitting_leaf])
            ...
            if len(leaves)>1:
                ...
                #Update the rates and distances
                #rates
                leaf_rates[current_node+1]=CalculateAllNewRates(tuple(prob*x + (1-prob)*y for x, y in zip(leaf_rates[hgt_acceptor_leaf], leaf_rates[hgt_donor_leaf])))
    ...
    #             #Do nothing, there is only one leaf.    
            ...
            #Update the rates and distances
            #rates
            leaf_rates[current_node+1]  =CalculateAllNewRates(tuple(prob*x + (1-prob)*y for x, y in zip(leaf_rates[hyb_pair[0]], leaf_rates[hyb_pair[1]])))
...
if taxa_limit:
    leaves = []
    no_of_extinct = 0
    while len(leaves)+c_ext*no_of_extinct!=taxa_limit:
        if not simple_output:
            print("starting over")
        network,hybrid_nodes,leaves,no_of_extinct = CalculateNetwork()
else:
    network,hybrid_nodes,leaves,no_of_extinct = CalculateNetwork()
